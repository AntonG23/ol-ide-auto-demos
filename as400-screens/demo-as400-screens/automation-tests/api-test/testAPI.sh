...
# Build & run the api
find ../../sample-project/ -name "pom.xml" -exec grep -l 'provider\|DB.Driver' {} + | xargs -I '{}' mvn clean install -DskipTests -f '{}' >> logs.txt
...
echo "Running newman"
newman run AS400-Screens.postman_collection.json -e as400-screens.postman_environment.json --reporters cli,junit --reporter-junit-export ../../results/api-test/api-test-results.xml --disable-unicode
status=$?
